...
public class UserController {
    ...
    @PostMapping("/fix-admin-bcrypt")
    public ResponseEntity<?> fixAdminBCrypt() {
        try {
            System.out.println("=== FIXING ADMIN BCRYPT ISSUE ===");
            
            // Find the admin user
            Optional<User> adminOptional = userService.findByUsername("admin");
            if (adminOptional.isEmpty()) {
                return ResponseEntity.badRequest()
                    .body(createErrorResponse("Admin user 'admin' not found"));
            }
            
            User admin = adminOptional.get();
            System.out.println("Found admin: " + admin.getUsername());
            System.out.println("Current password: " + admin.getPassword());
            System.out.println("Current password length: " + admin.getPassword().length());
            System.out.println("Is BCrypt encoded: " + admin.getPassword().startsWith("$2"));
            
            // Fix: Re-encode the password with BCrypt
            String plainPassword = "admin123"; // Your actual password
            ...
            System.out.println("✅ FIXED: Password re-encoded with BCrypt");
            System.out.println("New encoded password: " + encodedPassword);
            System.out.println("New password length: " + encodedPassword.length());
            
            // Test the encoding
            boolean testResult = passwordEncoder.matches(plainPassword, encodedPassword);
            System.out.println("Password test: " + testResult);
            
            Map<String, Object> response = new HashMap<>();
            response.put("message", "Admin password fixed with BCrypt encoding");
            response.put("username", "admin");
            response.put("password", "admin123");
            response.put("encodingTest", testResult);
            response.put("oldPasswordLength", admin.getPassword().length());
            response.put("newPasswordLength", encodedPassword.length());
            
            return ResponseEntity.ok(response);
            
        } catch (Exception e) {
            System.err.println("Error fixing admin BCrypt: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.status(500)
                .body(createErrorResponse("Failed to fix admin: " + e.getMessage()));
        }
    }
}